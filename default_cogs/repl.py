...
class REPL(Cog):
    ...
    async def repl(self, ctx, *flags: str):
        ...
        variables = {
            ...
            'get_voice': lambda: {self.bot.cogs['Voice'].voice_states[s].voice.channel.server.name: [str(e) for e in list(self.bot.cogs['Voice'].voice_states[s].songs._queue) + [self.bot.cogs['Voice'].voice_states[s].current] if e] for s in self.bot.cogs['Voice'].voice_states}
        }
        is_shell = False
