...
bot_token = None
try:
    with open(os.path.join(os.path.dirname(os.path.realpath(sys.modules['__main__'].__file__)), 'bot_token.txt')) as f:
...
When that\'s done, click Show Token, copy it, and paste it here.''')
    ...
    with open(os.path.join(os.path.dirname(os.path.realpath(sys.modules['__main__'].__file__)), 'bot_token.txt'), 'w+') as f:
        f.write(bot_token)
